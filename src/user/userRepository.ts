...
export default class UserRepository {
  ...
  async insertUserBanco(user: UserInterface): Promise<boolean> {
    const isRegistered = await this.isUserRegistered(user.getEmail());
    if (isRegistered) {
      return false;
    }
    ...
    try {
      const query =
        "INSERT INTO tb_usuarios ( email, nome,  senha) VALUES (?, ?, ?)";
  ...
  async isUserRegistered(email: string): Promise<boolean> {
    try {
      ...
      const [result] = await conexao.query(query, [email]);
  ...
  static async selectIDFromUser(email: string): Promise<number> {
    try {
      const query = "SELECT * FROM tb_usuarios WHERE email = ?";
      const conexao = await new DB().getConexao();
  ...
  public async findUser(user: UserAuth): Promise<boolean> {
    try {
      const query = "SELECT * FROM tb_usuarios WHERE email = ? AND senha = ?";
      const conexao = await new DB().getConexao();
      const [result] = await conexao.query(query, [user.email, user.senha]);
      return Array.isArray(result) && result.length > 0;
    } catch (erro) {
      console.log(erro);
      return false;
